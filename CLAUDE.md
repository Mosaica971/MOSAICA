...
> until the MAELIA training: the two MAELIA couplings.
>
> **MAELIA: read `docs/maelia/README.md` before any coupling or MAELIA question.** It digests
> the MAELIA user guide (PDFs in `context/maelia/`; grep the text extracts in
> `context/maelia/text/`, not the PDFs) and its § M says what it changes for the coupling: the plan is
> imposed through `parcelles.shp` `SEQUENCE` under `nomChoixAssolement='Donnees'`; the ITK is
> *determined* by the situation, not chosen, so a MOSAICA variant must be its own MAELIA
> species; AqYield scales down a `RENDEMENT_OPTIMAL` that is an input (vigilance C.1 is moved,
> not solved); tropical and perennial crops are absent from the guide. File formats, launcher
> parameters and output columns: `docs/maelia/reference.md`. **The coupling plan is
> `docs/maelia/coupling-todo.md`** (trunk I, then D yields and E robustness test; tick its
> boxes as work lands), and `docs/maelia/crop-parameters.md` says per crop what MAELIA needs
> that MOSAICA's tables give — its § 2 is generated by `scripts/build_maelia_crop_table.py`. What is left is in
