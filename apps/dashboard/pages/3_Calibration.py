...
the repo root with:  streamlit run apps/dashboard/app.py
"""
...
st.title("Calibration: observed 2017 vs simulated")
st.caption(
    "Gap measured at the level of the 12 observed RPG groups, after Chopin et al. (2015) "
    "section 2.6. The PAD is the percentage absolute deviation between observed and simulated "
    "area; it is 0 when the simulation reproduces the observation exactly."
)

runs = loaders.list_output_runs(OUTPUTS_ROOT)
if not runs:
    st.info("No run found in `outputs/`.")
    st.stop()
...
if pad_by_crop is None:
    st.warning(
        "This run has not been scored yet. Run:\n\n"
...
left.metric(
    f"Territorial PAD (threshold {thresholds.get('regional_pad_max', 15):.0f} %)",
    _pct(calib.get("regional_pad_pct")),
    _verdict(calib.get("regional_within_threshold", False)),
    delta_color="off",
)
middle.metric(
    f"Farm types (threshold {thresholds.get('farm_type_match_min', 80):.0f} %)",
    _pct(calib.get("farm_type_match_pct")),
    _verdict(calib.get("farm_type_within_threshold", False)),
    delta_color="off",
)
right.metric(
    "Plots correctly simulated",
    _pct(calib.get("plot_match_pct")),
    f"{_pct(calib.get('area_match_pct'))} of the area",
    delta_color="off",
)

st.subheader("Regional scale — area by crop")
regional = pad_by_crop.copy()
regional["crop_label"] = [
    key if key == TOTAL_KEY else label_for(str(key)) for key in regional["crop"]
]
st.dataframe(
    regional[["crop_label", "observed_ha", "simulated_ha", "abs_deviation_ha", "pad_pct"]]
...
st.subheader("Sub-regional scale — PAD (%) by region and crop")
...
st.subheader("Farm scale — observed vs simulated types")
confusion = loaders.load_calibration(run_dir, "farm_type_confusion")
if confusion is not None:
    matrix = confusion.set_index("observed_type")
    ...
    st.dataframe(matrix.style.background_gradient(cmap="Blues"), width="stretch")
    st.caption(
        "Rows: type observed in 2017. Columns: type after simulation. The diagonal is the "
        "share correctly reproduced (Chopin et al. 2015, Table 4)."
    )

st.subheader("Plot scale — agreement by sub-region")
...
st.subheader("Farm scale — PAD distribution")
...
if by_farm is not None:
    ...
    st.write(
        f"{within} farms out of {len(by_farm)} under the {threshold:.0f} % threshold."
    )
