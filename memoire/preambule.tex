...
\newtcolorbox{reflexion}[1][]{%
  ...
  fontupper = \small,
  title = {\itshape Retour rÃ©flexif},
...
% --- QUESTION / REPONSE : les questions laissees en marge de la redaction ---
% Encadre de TRAVAIL, pas de contenu : il porte une question posee dans le .tex
% (marqueurs "--- ... ---") et la reponse. Il est destine a DISPARAITRE une fois la
% reponse reformulee dans la prose -- ne pas le laisser dans la version deposee.
% Le .log les compte via "QUESTION :".
% Interrupteur : \questionsvisiblesfalse fait DISPARAITRE les 35 encadres sans toucher au
% texte -- c'est ainsi qu'on lit le vrai compte de pages du coeur, et c'est ce qu'il faudra
% basculer au depot si les reponses n'ont pas encore ete reformulees.
\newif\ifquestionsvisibles
\questionsvisiblestrue
\newtcolorbox{questionreponse}[1]{%
  enhanced, breakable, sharp corners,
  colback = cadreClimat!4, colframe = cadreClimat!65,
  boxrule = 0pt, leftrule = 2pt,
  left = 8pt, right = 8pt, top = 6pt, bottom = 6pt,
  fontupper = \small,
  before upper = {\typeout{QUESTION}},
  title = {\sffamily\bfseries\footnotesize QUESTION\ \textperiodcentered\ \normalfont\itshape #1},
  coltitle = black, colbacktitle = cadreClimat!14,
  fonttitle = \small,
  attach title to upper = {\\[3pt]},
}
% Le contenu est compose dans une boite jetee : rien ne s'imprime, et aucune commande du
% document (\ref, \gls, \SI) n'est desactivee -- donc rien ne casse.
\newsavebox{\poubellequestion}
\ifquestionsvisibles\else
  \renewenvironment{questionreponse}[1]%
    {\setbox\poubellequestion=\vbox\bgroup\hsize=\linewidth\begingroup\noindent}%
    {\par\endgroup\egroup}%
\fi
...
%     chapitre~\ref{ch:xxx}   Â§~\ref{sec:xxx}   tableau~\ref{tab:xxx}
...
\usepackage{titlesec}
% LE CHAPITRE NE COMMENCE PLUS SUR UNE PAGE NEUVE. `report` fait de \chapter un titre de
% classe "page" : saut de page force + ~2,5 cm de blanc avant le titre. Sur SEPT chapitres et
% un budget de 30 pages, cela coute 4 a 5 pages de blanc -- soit 15 % du memoire, pour une
% convention de livre qui n'a aucun sens sur un chapitre de deux pages. `straight` les enchaine
% comme des sections, ce qui est la composition normale d'un rapport de trente pages, et
% conserve toute la machinerie \ref / \appendix / \chaptermark.
\titleclass{\chapter}{straight}
\titleformat{\chapter}[block]
  {\normalfont\Large\bfseries}{\normalsize\mdseries\textsc{\chaptertitlename~\thechapter}\\[2pt]}{0pt}{\Large}
\titlespacing*{\chapter}{0pt}{20pt plus 6pt minus 2pt}{12pt}
...
\newcommand{\arediger}[1]{%
  ...
  \par\medskip\noindent\fbox{\parbox{\dimexpr\linewidth-2\fboxsep-2\fboxrule}{%
    \small\itshape Ã€ rÃ©diger --- #1}}\par\medskip}

% --- ATTENTE : les quatorze questions posees aux encadrants -----------------
% Quatorze questions ont ete envoyees a J.-M. Blazy et L. Guinde (memoire/encadrants.md).
% PRINCIPE : le memoire est REDIGE SANS LEURS REPONSES, et il est deposable en l'etat.
% Chaque marqueur signale une phrase que la reponse ENRICHIRA -- jamais un trou qui
% empeche de deposer. La prose qui precede un marqueur dit donc ce qui est vrai
% aujourd'hui ("a ma connaissance", "le depot n'en garde pas trace"), et la reponse
% viendra la remplacer ou la preciser, pas la combler.
%
%   \attente{3}         marqueur discret en fin de phrase -> "[Q3]"
%   \attentebloc{1}{..} bloc a la place duquel un paragraphe entier s'ecrira
%
% Basculer \attentesvisiblesfalse avant le depot fait disparaitre les marqueurs sans
% toucher a la prose. Le .log les compte : "ATTENTE : Qn".
\newif\ifattentesvisibles
\attentesvisiblestrue
\newcommand{\attente}[1]{%
  \typeout{ATTENTE : Q#1}%
  \ifattentesvisibles\,{\scriptsize\sffamily\bfseries[Q#1]}\fi}
\newcommand{\attentebloc}[2]{%
  \typeout{ATTENTE : Q#1 (bloc)}%
  \ifattentesvisibles
    \par\smallskip\noindent
    \begin{tcolorbox}[enhanced,breakable,sharp corners,colback=white,colframe=black!35,
      boxrule=0.4pt,left=5pt,right=5pt,top=2pt,bottom=2pt,fontupper=\scriptsize]
      \textbf{\sffamily En attente --- question~#1 aux encadrants.} #2
    \end{tcolorbox}
    \par\smallskip
  \fi}

% --- MATIERE : la liste des items du corpus qui reviennent a une section ----
% Ce n'est PAS du contenu : c'est l'echafaudage de redaction. Chaque section porte
% l'inventaire de ce qu'elle doit dire (items du corpus, chiffres disponibles,
% fichiers joints), on redige EN DESSOUS, puis on supprime l'encadre.
% Compte dans le .log via `MATIERE :`, comme \arediger, pour que la progression
% se mesure au lieu de s'estimer.
\newtcolorbox{matiereboite}[1][]{%
  enhanced, breakable, sharp corners,
...
\newenvironment{matiere}[1][]{%
  \typeout{MATIERE : #1}%
  \begin{matiereboite}%
  \noindent{\scriptsize\bfseries\sffamily MATIÃˆRE\ \textperiodcentered\ #1}\par\vspace{3pt}%
  \begin{itemize}[leftmargin=1.2em,itemsep=1.5pt,topsep=1pt,parsep=0pt]%
}{%
  \end{itemize}%
  \end{matiereboite}%
}
% Identifiant d'item du corpus (memoire/corpus/*.yaml) : \idc{CAL-74}
\newcommand{\idc}[1]{{\scriptsize\sffamily\bfseries #1}}
% Macro de chiffres.tex disponible mais pas encore citee : \mac{padRetenuCS}
\newcommand{\mac}[1]{{\ttfamily\scriptsize\textbackslash #1}}
% Fichier joint (figure, table, script, source) : \fic{figures/xxx.pdf}
\newcommand{\fic}[1]{{\ttfamily\scriptsize #1}}
% Etiquettes de tete de puce
