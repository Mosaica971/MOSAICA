# Journal de vigilance (archive)

> **La synthèse actionnable est [`docs/04-vigilance.md`](../04-vigilance.md).** Ce fichier-ci
> est le **journal complet des enquêtes**, conservé parce qu'il porte ce que la synthèse ne peut
> pas : les chemins essayés puis abandonnés, avec la mesure qui les a écartés. C'est ce qui
> évite de refaire trois fois la même impasse — la recalibration de l'aversion au risque, les
> plafonds calés sur l'observé, la contrainte de cheptel.
>
> Il n'est plus tenu à jour en tant que tel : une nouvelle limite se documente dans `04`, et une
> nouvelle enquête dans une spec de `docs/superpowers/specs/`.
...
### Majeur — Quatre pièges découverts en portant les indicateurs manquants (2026-08-01)

**1. `KNO3` porte `AZOTE = 0` alors que le nitrate de potassium titre ~13 % d'azote.** Vu en
adossant la lecture P/K à la colonne existante : sur les 15 engrais nommés en triplet NPK,
l'azote déduit du nom égale `AZOTE` **au millième près** — la convention du fichier est donc
sûre. Mais les sels simples nommés chimiquement échappent au contrôle, et `KNO3` y est à zéro.
Non corrigé (mandat de parité) ; l'azote de ce produit est donc absent partout, y compris dans
les chiffres antérieurs à ce chantier.

**2. Le « bio » par itinéraire inclut la prairie.** `PN_PIQ` et `PN_TOUR` utilisent
`PROC_BIO_BOVIN`, donc l'indicateur de surface bio les compte — fidèlement à l'encodage. Sur
`output_3` **toute** la surface « bio » (6 096 ha) est du plancher de prairie, et la surface bio
**hors prairie vaut 0 ha**. D'où la variante `surface_bio_hors_prairie_*`, qui est celle à lire
pour une affirmation sur les terres cultivées.

**3. Rpest donne un plancher de ~2,4 aux cultures sans pesticide.** Une culture qui n'applique
aucun produit somme `ADI = 0` et `AQUATOX = 0` ; or les seuils font du **bas** de l'ADI le côté
défavorable, si bien que « aucun produit » s'encode exactement comme « le produit le plus
toxique ». Mesuré : prairie, jachère et `MA_PLBIO` à 2,36 contre 8,81 pour la banane intensive.
**Le classement est solide et c'est à quoi sert l'indicateur** ; le niveau absolu du bas
d'échelle ne l'est pas.

**4. La moyenne pondérée de Rpest n'en est pas une.** Le GAMS annonce des propriétés
« pondérées par les quantités de produits utilisés », mais la dose se simplifie entre numérateur
et dénominateur : chaque terme se réduit à la propriété elle-même, et la somme sur les opérations
est une **somme simple**. Une culture faisant beaucoup de petits traitements score donc plus haut
qu'une autre en faisant un seul gros, à quantité égale. Porté tel quel.

### Majeur — La carte parcellaire repose sur une jointure reconstruite
_Ouvert le 2026-08-01 avec la page Carte._

`data/gis/01_RPG 2017/` et `data_parc` décrivent le **même univers parcellaire** — 24 734
enregistrements de part et d'autre pour la couche dédoublonnée, 4 638 exploitations des deux
côtés avec la **même distribution exacte des tailles**, et zéro signature d'exploitation orpheline
— mais ils n'ont **aucun identifiant commun** : le modèle utilise `P1..P24734`, inventés en amont.
La jointure est donc reconstruite par signature d'exploitation (multiensemble des couples
commune + surface), puis parcelle à parcelle dans l'exploitation appariée.
→ **Couverture mesurée : 99,4 % des parcelles** (24 582), 96,7 % des exploitations. Contrôle
indépendant : l'emprise de chaque polygone est toujours ≥ la surface déclarée (ratio médian 1,74,
0 anomalie sur 2 000).
→ **Ce qu'elle ne permet pas.** 152 exploitations partagent une signature identique et restent
non appariées plutôt que devinées ; et **~1 557 parcelles** partagent commune ET surface avec une
voisine de la même exploitation, donc peuvent avoir été interverties. Les lectures territoriales
et régionales sont fiables ; **une parcelle isolée ne fait pas preuve**. La page l'affiche.

### Critique — Les 25 variantes maraîchères « Karusmart » sont des copies identiques
_Mesuré le 2026-07-31. Corrigé dans les scénarios prospectifs (la mise en garde vit désormais
dans `crop_groups.yaml`, entrée `bio_maraichage`) ; l'entrée reste ouverte parce que c'est un
**trou de données**, pas un bug de code._

`MA_TO_CO_JA` et les 24 `MA_{BAG,BRF,PAI}_{BIO,VEG,FER,NON}_{I,NI}` sont **identiques au bit
près** sur tous les paramètres que le modèle lit : marge 20 825,25 €/ha, variance 0,70,
rendement 36 t/ha, 1 128 h/ha, 75,6 kg N/ha, IFT 10, GES 1 525, eau 1 800 mm, carbone 1,94,
subvention 0, coût 18 774,75. Leurs empreintes d'éligibilité sont identiques aussi. Autrement
dit **les distinctions paillage (bagasse/BRF/paille), fertilisation (bio/végétal/fertilisant/
aucun) et irrigation ne portent aucune donnée** : ce sont 25 exemplaires d'une seule activité
conventionnelle. C'est la forme concrète du blocage déjà connu (« le bio n'est pas identifiable
proprement dans les données », spec `2026-07-17-bio-share-indicator-design-DEFERRED`), et c'est
sans doute pourquoi le GAMS les supprime en bloc (`Eq_MA_EXP_SUPP`).

**Deux conséquences, les deux ont mordu.** (a) Les rouvrir ajoute **548 000 binaires de pure
symétrie** (879 162 contre 331 044) : des copies interchangeables sont le pire cas pour un
branch-and-bound, qui explore indéfiniment des permutations équivalentes de la même solution.
(b) Toute **part de bio** bâtie sur elles est un **no-op** : le solveur satisfait « 25 % de bio »
en choisissant la copie nommée `_BIO_` d'une activité identique, à coût exactement nul. Une
...
3 882 / 3 498 €/ha contre 27 929 à `MA_ROTA` (277 kg N, IFT 15). L'arbitrage est réel, et c'est
sur elles seules que `bio_maraichage` est désormais bâti.
→ **Ne pas rouvrir `ma_exp_supp`** sans que les 25 variantes aient reçu des ITK distincts.
`scripts/check_scenario_feasibility.py` détecte et signale désormais tout groupe de cultures
indistinguables avant chaque batch.

### Majeur — Comparer des scénarios : trois métriques qui trompent si on les prend telles quelles
_Ouvert le 2026-07-31 avec la page Prospective. Les correctifs sont livrés ; l'entrée reste
ouverte parce que ce sont des pièges de **lecture**, que le code signale mais ne peut pas
supprimer._

**1. Un indicateur fixé par une contrainte est une hypothèse du scénario, pas son résultat.**
C'est le chantier des contraintes d'indicateurs qui a créé le problème : noter P8 sur les
subventions alors qu'il plafonne lui-même son budget à 80 M€ mesure ce qu'on lui a imposé, et
P1 remporte l'axe « dépense publique » en la mettant à zéro. `comparison.bound_indicators()`
les détecte depuis `recap["constraints"]` et la page avertit — mais **le score les inclut quand
même si on le demande**, à dessein : les exclure d'office empêcherait de voir l'effet d'un
plafond sur les autres indicateurs.

**2. Comparer des totaux entre scénarios qui n'allouent pas la même surface confond échelle et
intensité.** Une politique qui cultive moins d'hectares affiche moins d'azote total sans être
plus propre. Le bloc `intensity` donne les formes **par tonne** et par hectare ; c'est celles-là
qu'il faut lire pour une comparaison environnementale.

**3. « Résilience » désigne deux choses opposées dans ce dépôt.** `recap["resilience"]` est de
l'**exposition** : choc appliqué après le solve à une allocation figée. `core/reporting/
robustness.py` est de l'**adaptation** : le modèle a ré-optimisé sous chaque forçage. Les
additionner dans un score composite produirait un nombre sans signification. Voir aussi
l'entrée « Le score de stabilité mesure l'exposition, pas l'adaptation ».
→ **Trois conventions du module de robustesse, à connaître avant d'interpréter** : une politique
infaisable sous un forçage n'a **pas** de pire cas chiffré (la case est vide, sinon le pire de
ses survivants la ferait passer pour robuste) ; les forçages ne sont **pas moyennés** par défaut
(F10 « conjoncture favorable » et F9 « crise systémique » n'ont pas la même vraisemblance, et
une moyenne affirmerait une distribution que personne n'a choisie) ; le regret se mesure contre
la meilleure politique **du même forçage**, pas contre un optimum global.

### Majeur — Scénarios prospectifs : trois pièges qui ne se voient qu'après le solve
_Ouvert le 2026-07-31. Le fichier a été scindé le 2026-08-01 en trois catalogues
(`scenarios_politiques.yaml`, `scenarios_forcages.yaml`, `scenarios_pareto.yaml`) et un plan
(`plan.yaml`) ; les mises en garde chiffrées vivent en tête du catalogue de politiques. Spec :
`docs/superpowers/specs/2026-07-31-scenarios-prospectifs-design.md`._

**1. Le plafond de main d'œuvre est le vrai facteur limitant du modèle.** `Eq_MO_MAX_Expl`
accorde 6 252 740 h au territoire, soit **3 891 ETP**, pour 5 629 390 h utilisées dans
`output_3`. Deux conséquences qu'aucune relecture de YAML ne rend évidentes : un **plancher
d'emploi** au-dessus de 3 891 ETP est infaisable si le scénario ne desserre pas le `slack` ; et
les **planchers de production vivrière** réclament 6 222 537 h par leurs cultures les moins
gourmandes, soit 0,5 % de marge sur une borne inférieure qui ignore la concurrence foncière —
ils sont infaisables à `slack: 1.0` (`main.py` a échoué exactement là-dessus le 2026-07-21).
Un test sans données (`test_prospective_labour_slack_covers_every_employment_floor`) vérifie
désormais la cohérence des deux blocs.
→ **Et desserrer le `slack` ne suffit pas.** P10 s'est révélé infaisable avec un plancher à
6 000 ETP alors que son `slack: 2.2` autorisait 8 561 ETP de budget horaire. Ce ne sont pas les
heures qui manquaient mais les **cultures capables de les absorber** une fois les plafonds
environnementaux en vigueur. Maximiser le travail sous les autres contraintes de chaque politique
donne le vrai plafond : **P8 → 4 875 ETP, P10 → 4 975 ETP** (relaxation LP, donc bornes
supérieures), contre 3 503 ETP dans `output_3`.
→ **Attention à l'explication trop rapide.** L'intuition « travail et IFT sont corrélés » est
vraie dans la table des taux (maraîchage 1 128-1 653 h/ha à IFT 10-15 ; canne 12,7 h/ha à IFT
2,5 ; prairie 126 h/ha à IFT 0) mais elle **ne suffit pas** à expliquer l'infaisabilité : la
bissection montre que retirer le plafond d'IFT la laisse infaisable, tout comme retirer le
plafond d'azote ou celui de GES. C'est la combinaison des plafonds *et* la forme par ferme du
plafond de main d'œuvre qui bornent l'emploi, pas un levier isolé. Ne pas relever un plancher
d'emploi sans re-mesurer.

**2. Rouvrir `ma_exp_supp` triple la taille du modèle.** Les 25 variantes maraîchères
expérimentales sont les systèmes paillés (bagasse / BRF / paille × fertilisation), c'est-à-dire
précisément les systèmes agroécologiques du jeu de données — les rouvrir est agronomiquement
justifié. Mais on passe de ~309 000 à **879 162 binaires**, soit exactement le régime où HiGHS
cesse de trouver un bon *incumbent* seul (cf. l'entrée warm start). Toute politique qui les
rouvre demandera un `time_limit` relevé et/ou un warm start.

**3. L'indicateur `eau` du modèle et celui du rapport ne mesurent pas la même chose.** Les taux
sont par culture, alors que seule une parcelle irrigable prélève : sans `plot_weight: irrigable`,
un plafond d'eau compte les hectares pluviaux et vaut **56 Mm³** là où le recap rapporte
**35 Mm³**. Un seuil lu sur `recap.json` et posé sans le poids serait faux d'un facteur 1,6. Même
nature de piège pour `carbone` : le taux exposé au modèle est l'**entrée** de carbone de la
culture, tandis que le bilan rapporté dépend aussi du type de sol de la parcelle (`Data_Sol`),
qu'un taux par culture ne peut pas porter.
...
**RÉSOLU le 2026-07-29 par le warm start** (`core/solve/warm_start.py`, cf. `TODO.md`). Le
...
### Mineur — `zone_indicator_bound` exempte silencieusement une zone sans terme
_Relevé le 2026-08-01 en relisant les contraintes génériques._

Une zone (ferme, bassin, région) dont aucune paire `(parcelle, culture)` éligible ne porte de
taux non nul n'apparaît pas dans `zone_terms`, donc **aucune contrainte n'est posée pour
elle**. Sans conséquence pour un **plafond** (zéro passe sous n'importe quel seuil positif) ;
mais pour un **plancher**, cette zone est dispensée au lieu de rendre le run infaisable.

C'est le même arbitrage que `cs_gfa_minimum_share` fait explicitement — perdre 3 fermes sur
4 588 plutôt que la contrainte entière — à ceci près qu'ici c'est le **seul** comportement
offert et qu'il n'était pas écrit. Commenté dans `core/model/constraints.py` depuis.
→ Le contrôle à faire avant de croire un plancher par zone : comparer le nombre de zones que
la contrainte a effectivement produites au nombre de zones que le groupement déclare.
...
- **Les runs de référence étaient antérieurs à quatre blocs de recap** (2026-08-01) —
  `output_1`/`output_2`/`output_3` ne portaient ni `intensity`, ni `agroecology`, ni les
  nutriments P/K, ni Rpest : la page Prospective et une partie du score composite étaient
  vides sur eux. Les deux calibrations ont été **régénérées** depuis
  `scenarios_calibration.yaml` sous les noms `calib_gams_parite` et `calib_retenu`, et
  reproduisent leurs valeurs documentées **au chiffre près** — objectif 84 978 687 / PAD
  48,44 / types 64,04 d'un côté, 81 222 224 / 6,60 / 86,89 de l'autre, les deux `optimal`.
  Ce qui vaut confirmation indépendante que la restructuration de la matinée n'a rien changé
  au comportement du solve (le golden ne couvre pas la résolution).
  → `outputs/reference_2017/` **n'a PAS été reconstruit** : il ne dépend ni des contraintes ni
  des blocs de recap, seulement de `year` / `zone_filter` / `baseline_representative_crops`,
  qu'aucun de ces changements ne touche. C'est la règle de `CLAUDE.md`, appliquée telle quelle.
  → `output_1`/`output_2`/`output_3` sont conservés : `output_2` (plafond plantain seul, PAD
  31,7 %) est l'état intermédiaire, que la nouvelle paire ne reproduit pas.
- **Aucun garde-fou sur le solve** (2026-08-01) — les tests sont sans données par choix et
  `golden_snapshot` s'arrête avant la résolution : rien ne remarquait qu'un refactor ou une
  édition de config déplaçait le PAD. `scripts/check_references.py` compare désormais les
  chiffres publiés ici aux chiffres que les dossiers contiennent (aucun solve, instantané).
  Tolérance 5 %, choisie **au-dessus du bruit de branch-and-bound mesuré** — 0,15 point de PAD
  sur trois graines HiGHS, soit 2,3 % en relatif sur la plus petite valeur gardée. Une garde à
  l'égalité exacte échouerait sur un simple re-run.
- **`allocation_diff` reproduit trois constats de ce fichier sans les connaître** (2026-08-01)
  — le diff d'allocation entre les deux calibrations sort `CS → PN` 2 972 ha, `BC → BA`
  1 189 ha, et à résolution fine `CS_MG_NISM → PN_PIQ` 1 069 ha. À comparer aux entrées
  « déficit de prairie » (« 83 % en canne (2 881 ha) », « la seule variante `CS_MG_NISM`
  absorbe 1 042 ha ») et « plantain » (« banane export → plantain, 1 298 ha »). Trois mesures
  indépendantes qui retombent sur les mêmes chiffres à la contrainte près.
- **Deux `_code_key` divergents pour la même chose** (2026-08-01) — `domain/zones.py` et
  `dashboard/comparison.py` normalisaient chacun un code région/île vers sa clé de libellé,
  par deux algorithmes différents : découpe de chaîne (`"4.5"` → `"4.5"`) d'un côté,
  `str(int(float(·)))` (`"4.5"` → `"4"`) de l'autre. Deux libellés possibles pour la même
  région selon la fonction appelée. Fusionné sur `zones.code_key`, `comparison.label_region`
  / `label_island` sont désormais des **alias** et non des copies, avec un test qui l'exige.
- **Arborescence : `apps/` et `core/solve/`** (2026-08-01) — le dashboard ne participe pas au
  solve (il ne lit que des `outputs/output_N/`), il est sorti de `case_studies/guadeloupe/`
  vers `apps/dashboard/` ; et `core/model/` mélangeait construire un modèle et le résoudre,
  d'où `core/solve/` (`solver`, `progress`, `timing`, `warm_start`, `shadow_prices`).
  Invariance vérifiée : `golden_snapshot.py --check` inchangé sur ses 681 sommes de contrôle.
- **Quatre plans exécutés supprimés** (2026-08-01) — convention de `CLAUDE.md` (« only
  unexecuted plans are kept ») appliquée : ~210 ko, chacun avec sa spec conservée.
- **Doublon `_n` entre deux scripts** (2026-08-01) — même formateur de nombres recopié dans
  `build_reference_state.py` et `compare_to_reference.py`, désormais `_common.format_number`.
  Au passage `CONFIG_PATH` avait deux sources (`_common` et `data_pipeline`) : la
  restatement est conservée pour ne pas charger pandas au `--help`, mais un test exige
  qu'elles s'accordent.
- **BOM UTF-8 sur `2_Comparaison.py`** (2026-08-01) — invisible à l'exécution (Python
  décode en `utf-8-sig`) mais faisait échouer tout outillage lisant le fichier en `utf-8`
  pur, `ast.parse` compris. Retiré ; aucun autre fichier du dépôt n'en portait.
- **`GeometryJoin.coverage`** (2026-08-01) — propriété nommée « couverture » qui renvoyait un
  **effectif** et non une part, et qu'aucun appelant n'utilisait. Supprimée ; `summary()`
  calcule bien la part, elle.
