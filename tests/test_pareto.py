...
def _recap(name, azote, margin):
    return {
        "run_name": name,
        "environment": {"output": {"total_nitrogen": azote}},
        "economics": {"output": {"total_gross_margin": margin}},
    }


def _sweep(points, sweep="pareto_nitrogen"):
...
def test_sweep_name_strips_the_matrix_suffix():
    assert pareto.sweep_name("pareto_nitrogen__threshold=1544722") == "pareto_nitrogen"
    assert pareto.sweep_name("calib_selected") == "calib_selected"


def test_sweeps_in_groups_runs_by_their_prefix():
    recaps = {**_sweep([(1, 10, 100)]), **_sweep([(2, 20, 200)], sweep="pareto_employment")}

    assert pareto.sweeps_in(recaps) == ["pareto_employment", "pareto_nitrogen"]
...
def test_sweep_of_prefers_the_recap_coordinates_over_the_name_prefix():
    ...
    recap = _recap("P8_transition__pareto_nitrogen__threshold=1544722", 10, 100)
    recap.update(run_policy="P8_transition", run_forcing=None, run_sweep="pareto_nitrogen")

    assert pareto.sweep_of(recap) == "P8_transition__pareto_nitrogen"


def test_sweep_of_separates_the_same_front_traced_under_different_forcings():
    nominal = _recap("P8__pareto_nitrogen__threshold=1", 10, 100)
    nominal.update(run_policy="P8", run_forcing=None, run_sweep="pareto_nitrogen")
    forced = _recap("P8__F9__pareto_nitrogen__threshold=1", 9, 80)
    forced.update(run_policy="P8", run_forcing="F9", run_sweep="pareto_nitrogen")

    assert pareto.sweep_of(nominal) != pareto.sweep_of(forced)
    assert pareto.sweeps_in({"a": nominal, "b": forced}) == [
        "P8__F9__pareto_nitrogen", "P8__pareto_nitrogen"
    ]
...
def test_sweep_of_falls_back_to_the_name_for_runs_predating_the_coordinates():
    ...
    assert pareto.sweep_of(_recap("pareto_nitrogen__threshold=1544722", 10, 100)) == "pareto_nitrogen"
...
def test_collect_points_reads_both_axes_and_sorts_along_x():
    ...
    points = pareto.collect_points(recaps, "pareto_nitrogen", "total_nitrogen",
                                   "total_gross_margin")
...
def test_a_run_missing_an_indicator_is_skipped_rather_than_placed_at_zero():
    ...
    recaps["pareto_nitrogen__threshold=2"] = {"run_name": "pareto_nitrogen__threshold=2"}

    points = pareto.collect_points(recaps, "pareto_nitrogen", "total_nitrogen",
                                   "total_gross_margin")

    assert [p.label for p in points] == ["pareto_nitrogen__threshold=1"]
...
def test_a_well_behaved_front_has_no_dominated_point():
    ...
    marked = pareto.mark_dominated(points, "total_nitrogen", "total_gross_margin")
...
def test_a_point_worse_on_both_axes_is_dominated():
    ...
    assert dominated == ["pareto_nitrogen__threshold=2"]


def test_domination_respects_each_indicators_own_direction():
    """Both axes are costs here (nitrogen and IFT), so LOW wins on both."""
    recaps = {
        "s__a": {"run_name": "s__a", "environment": {"output": {"total_nitrogen": 100.0,
                                                                "total_tfi": 10.0}}},
        "s__b": {"run_name": "s__b", "environment": {"output": {"total_nitrogen": 200.0,
                                                                "total_tfi": 20.0}}},
    }
    points = pareto.collect_points(recaps, "s", "total_nitrogen", "total_tfi")

    marked = pareto.mark_dominated(points, "total_nitrogen", "total_tfi")
...
def test_marginal_rates_give_the_slope_between_consecutive_front_points():
    ...
    points = pareto.mark_dominated(
        pareto.collect_points(recaps, "pareto_nitrogen", "total_nitrogen", "total_gross_margin"),
        "total_nitrogen", "total_gross_margin",
    )
...
def test_marginal_rates_skip_dominated_points():
    ...
    assert [(r["from"], r["to"]) for r in rates] == [
        ("pareto_nitrogen__threshold=1", "pareto_nitrogen__threshold=3")
    ]


def test_the_figure_builds_on_an_empty_sweep_without_raising():
    figure = pareto.build_front_figure([], "total_nitrogen", "total_gross_margin")
