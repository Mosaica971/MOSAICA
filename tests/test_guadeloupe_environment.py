...
compute_crop_variable_cost_per_ha; formulas are OPTIMISATION.txt lines 98-127.
"""
...
def _otk_fixture():
    ...
    crop_operation_matrix = pd.DataFrame({"C1": [1.0, 1.0]}, index=["OP_UP", "OP_AM"])
    crop_plantation_duration = pd.Series({"C1": 2.0})
    crop_cycle_duration = pd.Series({"C1": 6.0})  # /6*12 == *2
    crop_yield = pd.Series({"C1": 10.0})
    return operation_data, crop_operation_matrix, crop_plantation_duration, crop_cycle_duration, crop_yield


def test_azote_per_ha_cult_splits_amortized_and_annualizes():
    operation_data, matrice, dplant, dcycle, _ = _otk_fixture()
    # upfront DOSE*AZOTE = 2*3 = 6 ; amortized (1*4)/2 = 2 ; (6+2)/6*12 = 16
    result = environment.compute_crop_nitrogen_per_ha(operation_data, matrice, dplant, dcycle)
...
def test_ift_per_ha_cult_uses_ift_without_dose():
    ...
    result = environment.compute_crop_tfi_per_ha(operation_data, matrice, dplant, dcycle)
    assert result["C1"] == pytest.approx(4.0)


def test_ges_per_ha_cult_adds_production_term_upfront_only_and_divides_by_coeff():
    operation_data, matrice, dplant, dcycle, rdt = _otk_fixture()
    # surface: upfront 5 + amortized 6/2=3 -> (8)/6*12 = 16
    # production (upfront op only): GES_Q*Rdt = 0.1*10 = 1 -> /6*12 = 2
    # (16 + 2) / coeff(0.5) = 36
    result = environment.compute_crop_ghg_per_ha(
        operation_data, matrice, dplant, dcycle, rdt, coeff_c_co2=0.5
    )
