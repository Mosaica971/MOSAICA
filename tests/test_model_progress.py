...
def test_run_with_progress_runs_func_on_the_calling_thread():
    # Architectural guard: the solve must NOT be backgrounded. The persistent APPSI
    # solver loads the model inside capture_output(capture_fd=True); running it on a
    # worker thread while a progress bar writes from the main thread corrupts Pyomo's
    # process-global stdout/stderr fd state. See docs/superpowers/specs/
