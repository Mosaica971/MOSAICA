...
def _totals():
    ...
    environment = {"total_nitrogen": 4000.0, "total_tfi": 250.0}
...
def test_land_and_labour_intensities():
    ...
    assert ratios["fte_per_ha"] == pytest.approx(0.1)
    assert ratios["production_per_ha"] == pytest.approx(10.0)
    assert ratios["gross_margin_per_fte"] == pytest.approx(20_000.0)
...
def test_environmental_intensity_is_per_tonne_not_per_hectare():
    ...
    ratios = compute_intensity_totals(*_totals(), surface_ha=100.0)
    assert ratios["nitrogen_per_tonne"] == pytest.approx(4.0)
    assert ratios["tfi_per_tonne"] == pytest.approx(0.25)


def test_public_spending_efficiency():
    ratios = compute_intensity_totals(*_totals(), surface_ha=100.0)
    assert ratios["subsidy_per_tonne"] == pytest.approx(50.0)
    assert ratios["subsidy_per_fte"] == pytest.approx(5000.0)
...
def test_zero_denominator_yields_none_not_zero():
    ...
    assert ratios["nitrogen_per_tonne"] == pytest.approx(4.0)  # unaffected by surface
...
def test_zero_production_and_zero_employment_are_handled():
    ratios = compute_intensity_totals(
        {"total_production_tonnes": 0.0, "total_subsidy": 10.0,
         "total_gross_margin": 0.0, "total_fte": 0.0},
        {"total_nitrogen": 5.0, "total_tfi": 1.0},
        surface_ha=10.0,
    )
    assert ratios["nitrogen_per_tonne"] is None
    assert ratios["subsidy_per_fte"] is None
    assert ratios["gross_margin_per_fte"] is None
