...
def test_cane_combo_of_parses_the_three_real_combos_and_ignores_non_cane():
    assert comparison.cane_combo_of("CS_BT_NISM") == "Rain-fed · semi-mechanised harvest"
    assert comparison.cane_combo_of("CS_NGT_NIM") == "Rain-fed · mechanised harvest"
    assert comparison.cane_combo_of("CS_NGT_IM") == "Irrigated · mechanised harvest"
    assert comparison.cane_combo_of("CF_NBT_NISM") == "Rain-fed · semi-mechanised harvest"
...
def test_stack_key_none_collapses_and_subculture_colors_cane_by_combo():
    ...
    assert comparison.stack_key("CS_BT_IM", "BT", 1, "subculture") == "Irrigated · mechanised harvest"
...
def test_pivot_measure_by_culture_stacked_by_subculture_colors_cane_by_combo():
    ...
    assert pivot.loc["CS", "Irrigated · mechanised harvest"] == pytest.approx(50.0)
    assert pivot.loc["CS", "Rain-fed · semi-mechanised harvest"] == pytest.approx(100.0)
...
def test_series_label_is_run_name_plus_side():
    assert comparison.series_label("baseline", "output") == "baseline · output"
    assert comparison.series_label("baseline", "input") == "baseline · input"


def test_series_labels_leaves_unique_base_labels_untouched():
    rows = [("baseline · output", "output_3"), ("free_melon · output", "output_11")]
...
def test_series_labels_suffixes_only_colliding_labels_with_folder():
    rows = [
        ("baseline · output", "output_3"),
        ("baseline · output", "output_9"),
        ("free_melon · output", "output_11"),
    ]
    assert comparison.series_labels(rows) == [
        "baseline · output (output_3)",
        "baseline · output (output_9)",
...
def test_composite_score_constant_column_scores_half():
    raw = pd.DataFrame(
        {"total_revenue": [100.0, 200.0], "total_ghg": [3.0, 3.0]},
        index=["S1", "S2"],
    )
    scores = comparison.compute_composite_scores(
        raw, {"total_revenue": 1.0, "total_ghg": 1.0}
...
def test_composite_score_ignores_zero_weight_columns():
    ...
    scores = comparison.compute_composite_scores(
        raw, {"total_revenue": 1.0, "total_ghg": 0.0}
    )
...
def test_composite_score_single_scenario_is_half():
    raw = pd.DataFrame({"total_revenue": [100.0], "total_ghg": [5.0]}, index=["S1"])
    scores = comparison.compute_composite_scores(
        raw, {"total_revenue": 1.0, "total_ghg": 1.0}
    )
...
def test_build_grouped_bar_figure_stacked_and_unstacked_return_figures():
    ...
    series = [("run_1 · output", p1), ("run_2 · output", p2)]

    stacked = comparison.build_grouped_bar_figure(
        series, measure_label="Revenue (€)", x_axis_label="Culture",
    ...
    unstacked = comparison.build_grouped_bar_figure(
        [("run_1 · output", u1)], measure_label="Surface (ha)", x_axis_label="Region",
...
def test_format_dim_value_maps_region_and_island_codes_to_names():
    ...
    assert comparison.format_dim_value("culture", "CS") == "Sugarcane"
...
def test_build_indicator_parallel_axes_figure_returns_figure_with_native_axes():
    ...
    raw = pd.DataFrame(
        ...
        index=["run_1 · output", "run_2 · output"],
    )
...
_CONFIG = {
    ...
    "labor": {"hours_per_fte": 1000.0, "cost_per_hour": 10.0},
}
...
def _dataset() -> Dataset:
    ...
    farm_plot_map = pd.DataFrame({"farm": ["E1", "E2"], "plot": ["P1", "P2"]})
    # farm_plots is consumed by calibration.farm_type_confusion, which generate_report runs
    # on every allocation; it recomputes the farm typology through compute_farm_type.
...
def test_facts_csv_written_by_report_round_trips_through_comparison_helpers(tmp_path):
    ...
    assert set(facts.columns) >= {"crop", "region", "island", "revenue", "fte"}

    pivot = comparison.pivot_measure(facts, "region", "revenue", "none")
    assert pivot.sum().sum() > 0
    fig = comparison.build_grouped_bar_figure(
        [("run · output", pivot)],
        measure_label="Revenue (€)", x_axis_label="Region",
